...
read_status_value() {
    ...
    grep "^${key}=" "${status_file}" 2>/dev/null |
        head -n 1 |
        cut -d'=' -f2- || true
}
...
read_stage() {
    ...
    stage_file="$(read_status_value "${status_file}" "stage_file")"
    ...
    stage="$(head -n 1 "${stage_file}" 2>/dev/null || true)"
...
main() {
    ...
    mapfile -t filtered_status_files < <(
        filter_status_files "${all_status_files[@]}"
    )
    ...
    case "${MODE}" in
        ...
        latest)
            ...
            for f in "${filtered_status_files[@]}"; do
                ...
                if [[ "${start_epoch}" =~ ^[0-9]+$ ]] &&
                   [ "${start_epoch}" -ge "${latest_epoch}" ]; then
