...
while IFS= read -r qiime_bin; do
    ...
    if [ -z "${release}" ] || [ "${release}" != "${REQUIRED_RELEASE}" ]; then
        continue
    fi
    ...
    if [[ "${env_path}" == /home/adprc/miniconda3/envs/* ]]; then
        ...
        BEST_ENV_SOURCE="named_conda_env"
        break
    fi
done < <(find_candidate_qiime_bins)

# 第二輪：找 nextflow cached env
if [ -z "${BEST_QIIME_BIN}" ]; then
    while IFS= read -r qiime_bin; do
        ...
        if [[ "${env_path}" == /home/adprc/nf_conda/* ]]; then
            ...
            BEST_VERSION="${version}"
            BEST_LABEL="$(determine_label "${BEST_ENV_PATH}" "${BEST_RELEASE}")"
            BEST_ENV_SOURCE="nextflow_cached_env"
            break
        fi
    done < <(find_candidate_qiime_bins)
fi
...
export QIIME_ENV_SOURCE="${BEST_ENV_SOURCE}"
...
echo "[INFO] QIIME_ENV_SOURCE          : ${QIIME_ENV_SOURCE}"
echo "[INFO] QIIME_ENV_PATH            : ${QIIME_ENV_PATH}"
echo "[INFO] QIIME_ENV_NAME            : ${QIIME_ENV_NAME}"
echo "[INFO] QIIME_BIN                 : ${QIIME_BIN}"

if [ "${QIIME_ENV_SOURCE}" = "nextflow_cached_env" ]; then
    echo "[WARN] 目前使用的是 Nextflow cached env，不建議長期作為正式分析環境。"
    echo "[WARN] 建議建立正式命名環境，例如：qiime2-${QIIME_REQUIRED_RELEASE}"
    echo "[WARN] 可使用：./shell_tools/bootstrap_qiime_named_env.sh \"${QIIME_ENV_PATH}\" \"qiime2-${QIIME_REQUIRED_RELEASE}\""
fi
